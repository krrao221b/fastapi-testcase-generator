...
class TestCaseService:
    ...
    async def save_test_case_as_new(
        self, base_test_case_id: int, payload: SaveAsNewTestCaseRequest
    ) -> SaveAsNewTestCaseResponse:
        ...
        try:
            ...
            merged_tags = (
                ...
                else list(getattr(base, "tags", []) or [])
            )
            ...
            create_dto = TestCaseCreate(
                ...
                preconditions=(
                    ...
                    else base.preconditions
                ),
                test_steps=(
                    payload.test_steps
                    if payload.test_steps is not None
                    else list(base.test_steps or [])
                ),
            ...
            try:
                ...
                logger.warning(
                    "Saved new test case but failed to store embedding",
            ...
            logger.error(
                "Failed to save test case as new",
